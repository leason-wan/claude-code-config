...
describe("ClaudeCodeRouterConfig", () => {
  ...
  beforeEach(() => {
    ...
    // Reset process.env and set language to English
  ...
  describe("constructor", () => {
    it("should correctly initialize configuration paths", () => {
      ...
      // Use path.normalize to handle cross-platform path separators
      const path = require('path');
      expect(config.homeDir).toBe("/mock/home");
      expect(config.configDir).toBe(path.normalize("/mock/home/.claude-code-router"));
      expect(config.configFile).toBe(
        path.normalize("/mock/home/.claude-code-router/config.json")
      );
      expect(config.pluginsDir).toBe(path.normalize("/mock/home/.claude-code-router/plugins"));
      expect(config.transformerFile).toBe(
        path.normalize("/mock/home/.claude-code-router/plugins/modelscope-transformer.js")
      );
    });
  ...
  describe("createConfigFile", () => {
    ...
    it("should contain correct configuration structure", async () => {
      ...
      expect(configContent.Providers[0].models).toEqual(["Qwen/Qwen3-Coder-480B-A35B-Instruct"]);
      
      // Check router configuration
      expect(configContent.Router.default).toBe("modelscope,Qwen/Qwen3-Coder-480B-A35B-Instruct");
      expect(configContent.Router.think).toBe("modelscope,Qwen/Qwen3-Coder-480B-A35B-Instruct");
      expect(configContent.Router.background).toBe("modelscope,Qwen/Qwen3-Coder-480B-A35B-Instruct");
      expect(configContent.Router.longContext).toBe("modelscope,Qwen/Qwen3-Coder-480B-A35B-Instruct");
    });
  });
  ...
  describe("createTransformerFile", () => {
    ...
    it("should create transformer file", async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith(
        ...
        expect.stringContaining("class ModelScopeTransformer")
      );
    });
    ...
    it("should contain correct transformer code", async () => {
      ...
      expect(content).toContain("class ModelScopeTransformer");
      expect(content).toContain('name = "modelscope"');
      expect(content).toContain("transformRequestIn");
      expect(content).toContain("module.exports = ModelScopeTransformer");
    });
  });
  ...
  describe("setup", () => {
    ...
    it("should successfully complete setup process with environment variable", async () => {
      process.env.MODELSCOPE_API_KEY = "env-test-key";
    ...
    it("should detect API Key in environment variable", async () => {
      process.env.MODELSCOPE_API_KEY = "test-key-from-env";

      await config.setup();

      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "MODELSCOPE_API_KEY environment variable detected"
        )
      );
    ...
    it("should prompt for API Key when environment variable is not present", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "MODELSCOPE_API_KEY environment variable not found"
        )
      );
  ...
  describe("getApiBaseUrl", () => {
    ...
    it("should return ModelScope URL for 'cn' region", () => {
      const result = config.getApiBaseUrl("cn");
      expect(result).toBe("https://api-inference.modelscope.cn/v1/chat/completions");
    });

    it("should return ModelScope URL for 'intl' region", () => {
      const result = config.getApiBaseUrl("intl");
      expect(result).toBe("https://api-inference.modelscope.cn/v1/chat/completions");
    });

    it("should return ModelScope URL for unknown region", () => {
  ...
  describe("promptForApiKey", () => {
    ...
    it("should re-prompt when empty API Key is provided", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining("cannot be empty, please try again")
      );
    });
  });
});
